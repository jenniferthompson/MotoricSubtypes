...
all.daily <- all.daily %>%
  ...
  mutate(mental.stat.subtype.imp =
         ...
         sed.hypo.del.today = factor(ifelse(is.na(hypo.del.today), NA,
                                     ifelse(hypo.del.today == 'Yes' &
                                              ((!is.na(tot.benz) & tot.benz > 0) |
                                                 (!is.na(tot.prop) & tot.prop > 0)), 1, 0)),
                                     levels = 0:1, labels = c('No', 'Yes')),
         sed.hypo.del.imp = factor(ifelse(is.na(hypo.del.imp), NA,
                                   ifelse(hypo.del.imp == 'Yes' &
                                            ((!is.na(tot.benz) & tot.benz > 0) |
                                               (!is.na(tot.prop) & tot.prop > 0)), 1, 0)),
                                   levels = 0:1, labels = c('No', 'Yes')),
...
Hypoactive "on sedation" indicates that the patient had at least one CAM+ assessment on a study day
on which the total 24h dose of benzodiazepines and/or propofol was greater than 0. Due to
...
```{r print_desc_pt_fig, results = 'asis'}
putHfig(plot(summaryM(coma.s.imp + del.s.imp + days.hypodel.imp + days.hypodel.exp ~ ptgroup,
                      data = all.oneobs.desc, continuous = 5), which = 'continuous'))
putHfig(plot(summaryM(days.sedhypodel.imp + days.sedhypodel.exp + days.hyperdel.imp +
                        days.hyperdel.exp ~ ptgroup,
                      data = all.oneobs.desc, continuous = 5), which = 'continuous'))
putHfig(plot(summaryM(days.bothdel.imp + days.bothdel.exp + days.onlyhypo.imp + days.onlyhypo.exp +
                        days.onlyhyper.imp + days.onlyhyper.exp ~ ptgroup,
                      data = all.oneobs.desc, continuous = 5), which = 'continuous'))
