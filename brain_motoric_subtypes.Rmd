...
table_nums(name = 'results_dcloc',
           caption = 'Motoric Subtypes and Other Covariates vs Discharge to Institution among Hospital Survivors')
table_nums(name = 'cognitive_missing', caption = 'Missingness for Cognitive Outcome Models')
table_nums(name = 'results_rbans', caption = 'Motoric Subtypes and Other Covariates vs Global Cognition')
table_nums(name = 'results_trailb', caption = 'Motoric Subtypes and Other Covariates vs Executive Function')
...
figure_nums(name = 'hypo_dcloc', caption = 'Hypoactive Delirium vs Discharge to Institution')
figure_nums(name = 'hyper_dcloc', caption = 'Hyperactive Delirium vs Discharge to Institution')
...
figure_nums(name = 'hypo_trailb', caption = 'Hypoactive Delirium vs Executive Function')
figure_nums(name = 'hyper_trailb', caption = 'Hyperactive Delirium vs Executive Function')
...
## -- (Same data set will be used for discharge location model) ------------------------------------
...
data.surv <- all.oneobs %>%
  filter(id %in% pts.survived) %>%
  select(id, hospdis.loc, died.ever.365, days.deathlast.365,
         one_of(base.covars), one_of(hosp.covars)) %>%
  ## Create indicator for whether patient was discharged home vs institution
  mutate(dc.home = factor(ifelse(is.na(hospdis.loc), NA,
                          ifelse(hospdis.loc == 'Home', 0, 1)),
                          levels = 0:1, labels = c('Discharged home', 'Discharged to facility')))

## -- Describe missingness -------------------------------------------------------------------------
miss.surv <- map_chr(c(base.covars, hosp.covars), covars.missing,
                     df = data.surv, covar.list = c(base.covars, hosp.covars)) %>%
  paste.covars.missing()

pts.miss.surv <- sum(!complete.cases(data.surv))

## -- Create string describing missingness among covariates in 12m-mortality model, all pts -------
any.miss.surv <- !(miss.death.12.all == '')
if(!any.miss.surv){
  miss.surv <- 'there was no missingness in any covariates used in the model.'
} else{
  miss.surv <- paste('missingness among covariates was as follows:', miss.surv)
}

```

### Missingness
Among the `r length(pts.survived)` hospital survivors eligible to be included in analysis,
`r miss.surv`. This missingness occurred in `r pts.miss.surv` unique patients (`r round((pts.miss.surv / n.surv.inhosp)*100)`% of hospital survivors).

Since there was so little missingness, we included only the `r n.surv.inhosp - pts.miss.surv` complete case records in this analysis.

```{r death_12_surv_model}
## Set datadist
datadist.surv <- datadist(data.surv)
options(datadist = 'datadist.surv')

## Create Surv object for outcome
Surv.death.12.surv <- with(data.surv, {
...
**General summary of motoric subtypes:** Having either motoric subtype of delirium on a given day
seems to have no relationship with the hazard of death in the 12 months following study enrollment
when looking at the cohort of `r n.surv.inhosp - pts.miss.surv` patients . There is no
...
interaction terms, it is important to note that in the table, hypoactive delirium is adjusted to
`r as.character(datadist.surv$limits['Adjust to', 'days.hypodel.imp'])` days, and hyperactive delirium is adjusted to `r as.character(datadist.surv$limits['Adjust to', 'days.hyperdel.imp'])` days. More detailed hazard ratios for the two motoric subtypes are shown in `r figure_nums('subtypehrs_death_12_surv', display = 'cite')`.
...
quants.hypodel.surv <- unique(quantile(data.surv$days.hypodel.imp, probs = adjto.probs))
quants.hyperdel.surv <- unique(quantile(data.surv$days.hyperdel.imp, probs = adjto.probs))
...
lowhigh.hypo.surv <- c(datadist.surv$limits['Low:effect', 'days.hypodel.imp'],
                       datadist.surv$limits['High:effect', 'days.hypodel.imp'])
lowhigh.hyper.surv <- c(0, quantile(data.surv$days.hyperdel.imp, probs = 0.9, na.rm = TRUE))
...
hrs.death.12.surv <- pmap(surv.hrs.arglist,
  ...
  mutate(var.main.label = paste(comp.level, 'vs.', ref.level,
                                tools::toTitleCase(label(data.surv[,var.main]))),
         var.int.label = label(data.surv[,var.int]),
...
We examined the association of both motoric subtypes of delirium with the odds of discharge to an institution (anywhere other than home) among patients who survived their hospital stay: Given that
the patient was well enough to be discharged alive, did the amounts of either/both delirium subtypes
have any association with discharge to institution?

## Model Choice and Covariates
Our outcome is dichotomous (discharged to institution vs discharged home), and all our covariates
were either measured at baseline or are summary variables reflecting the entire ICU stay. Therefore,
we used a straightforward logistic regression model including the following covariates.
...
## Fit model
model.dcloc <- lrm(dc.home ~ rcs(age.enroll, 3) + edu + iqcode.score.e +
                     rcs(charlson.score, 3) + rcs(mean.modsofa.icu, 3) +
                     rcs(mean.benz.cube, 3) + rcs(mean.op.cube, 3) +
                     rcs(mean.prop.cube, 3) + mean.dex.cube + mean.hal.cube +
                     rcs(coma.s.imp, 3) + rcs(days.hypodel.imp, 3)*days.hyperdel.imp,
...
## Overall Model Results
**General summary of motoric subtypes:** More hypoactive delirium during a hospitalization has a
significant association with a higher likelihood of being discharged to an institution vs home.
Hyperactive delirium, however, appears to have no meaningful relationship with institutionalization.
There is no meaningful effect modification between the two subtypes.

Results for all model covariates, adjusted to the median or mode of all other covariates, are
presented in `r table_nums('results_dcloc', display = 'cite')`; due to the presence of
interaction terms, it is important to note that in the table, hypoactive delirium is adjusted to
`r as.character(datadist.surv$limits['Adjust to', 'days.hypodel.imp'])` days, and hyperactive delirium is adjusted to `r as.character(datadist.surv$limits['Adjust to', 'days.hyperdel.imp'])` days. More detailed hazard ratios for the two motoric subtypes are shown in `r figure_nums('hypo_dcloc', display = 'cite')` and `r figure_nums('hyper_dcloc', display = 'cite')`.

*Note on `r table_nums('results_dcloc', display = 'cite')`*: For continuous covariates,
hazard ratios will be different depending on the points of comparison chosen, especially for
covariates allowed to have a nonlinear relationship with the hazard of death (indicated by df > 1).
We have chosen to compare the 75th percentile and the 25th percentile by default, since this is
often a clinically relevant comparison. For variables with very skewed distributions, where those
...
```{r dcloc_results}
## Data frame of model results
results.dcloc <- model.results(model.dcloc,
                               data.set = data.surv,
                               short.label = lt.shortlabs,
                               print.format = 'plain',
                               add.stats = c("Model L.R.", "R2")) %>%
...
names(results.dcloc) <- c('Covariate', 'Reference\n(usu. 25th %ile)',
                          'Comparison\n(usu. 75th %ile)', 'Hazard ratio\n(95% CI)',
                          'X^2^', 'df', 'P')

```

```{r dcloc_results_print, results = 'asis'}
pandoc.table(results.dcloc,
             justify = c('left', rep('right', ncol(results.dcloc) - 1)),
             split.cells = c(30, rep(Inf, ncol(results.dcloc) - 1)),
             split.tables = Inf, keep.line.breaks = TRUE)
...
get.hypo.predict.lrm <- function(modobj){
  as.data.frame(Predict(modobj, days.hypodel.imp, days.hyperdel.imp = quants.hyperdel.surv,
                        fun = plogis)) %>%
  select(days.hypodel.imp, days.hyperdel.imp, yhat, lower, upper) %>%
  mutate(outcome = all.vars(modobj$call)[1],
...
get.hyper.predict.lrm <- function(modobj){
  as.data.frame(Predict(modobj, days.hyperdel.imp, days.hypodel.imp = quants.hypodel.surv,
                        fun = plogis)) %>%
  select(days.hypodel.imp, days.hyperdel.imp, yhat, lower, upper) %>%
  mutate(outcome = all.vars(modobj$call)[1],
...
## rows = "__ months"
format.for.dcloc.plot <- function(df.list){
  df.list %>%
    bind_rows() %>%
    mutate(facetvar.f = factor(facetvar,
                               labels = unique(paste(gsub(' of| delirium', '',
                                                          label(data.surv[,int.var])),
                                                     facetvar,
                                                     sep = ' = '))))
}

predict.hypo.dcloc <- get.hypo.predict.lrm(model.dcloc) %>% format.for.dcloc.plot()
predict.hyper.dcloc <- get.hyper.predict.lrm(model.dcloc) %>% format.for.dcloc.plot()

hypo.dcloc.plot <- ggplot(data = predict.hypo.dcloc, aes(x = xvar, y = yhat)) +
  facet_wrap(~ facetvar.f, nrow = 1) +
  ...
  scale_y_continuous(name = 'Adjusted probability of discharge to institution') +
  theme(strip.text = element_text(size = 11))

hyper.dcloc.plot <- ggplot(data = predict.hyper.dcloc, aes(x = xvar, y = yhat)) +
  facet_wrap(~ facetvar.f, nrow = 1) +
...
#### `r figure_nums('hypo_dcloc')`
```{r hypo_dcloc_plot, results = 'asis', fig.width=10}
hypo.dcloc.plot

```
#### `r figure_nums('hyper_dcloc')`
```{r hyper_dcloc_plot, results = 'asis', fig.width=10}
hyper.dcloc.plot
...
# save(data.cog.3, data.cog.12, areg.cog.3, areg.cog.12, models.rbans, datadist.cog,
#      file = 'shawn_moreexamples.Rdata')
...
# Trailmaking B Scores (Executive Function)

We wanted to explore whether durations of the two motoric delirium subtypes were associated with
executive function among patients who were tested at 3 and 12 months after hospital discharge.
...
```{r model_trailb}
## -- Fit Trails B models --------------------------------------------------------------------------
## Argument list for Trails B models
trailb.arglist <- list(outcome = c('trail.b.tscore.3', 'trail.b.tscore.12'),
                       df = list(data.cog.3, data.cog.12),
                       aregobj = list(areg.cog.3, areg.cog.12))

models.trailb <- pmap(trailb.arglist, fmi.ols)

```

## Overall Results
**General summary of motoric subtypes:** A longer duration of hypoactive delirium is significantly associated with worse executive function scores at both three and twelve months after hospital
discharge; this association is nonlinear, so please see `r figure_nums('hypo_trailb', display = 'cite')` for the full effect. Duration of hyperactive delirium is not associated with executive
function; it is impossible to know if there is a true effect which we can't see due to low amounts of hyperactive delirium, or whether there truly is no association. There is no significant interaction between the two motoric subtypes for this outcome.

Results for all model covariates, adjusted to the median or mode of all other covariates, are
presented in `r table_nums('results_trailb', display = 'cite')`; due to the presence of
interaction terms, it is important to note that in the table, hypoactive delirium is adjusted to
`r as.character(datadist.cog$limits['Adjust to', 'days.hypodel.imp'])` days and hyperactive delirium
is adjusted to `r as.character(datadist.cog$limits['Adjust to', 'days.hyperdel.imp'])` days. `r figure_nums('hypo_trailb', display = 'cite')` and `r figure_nums('hyper_trailb', display = 'cite')` show the association of each motoric subtype and executive function, adjusted to various amounts of the other subtype.

*Note on `r table_nums('results_trailb', display = 'cite')`*: For continuous covariates,
differences in scores will be different depending on the points of comparison chosen, especially for
covariates allowed to have a nonlinear relationship with the test score (indicated by df > 1).
We have chosen to compare the 75th percentile and the 25th percentile by default, since this is
often a clinically relevant comparison. For variables with very skewed distributions, where those
...
```{r results_trailb}
## -- Get tables of results for Trails B models ----------------------------------------------------
## Combine results tables for each time point
results.trailb <- map(models.trailb, .f = model.results,
                      data.set = data.cog.3,
                      print.format = 'plain',
                      short.labels = lt.shortlabs) %>%
  map(move.stats) %>%
  map(ci.newline) %>%
  map(ital.rows) %>%
  reduce(full_join, by = c('label', 'low', 'high'))

names(results.trailb) <-
  c('Covariate', 'Ref. (usu.\\\n25th %ile)', 'Comp. (usu.\\\n75th %ile)',
    'Difference\\\n(95% CI),\\\n3 months', 'X^2^, 3m', 'df, 3m', 'P, 3m',
    'Difference\\\n(95% CI),\\\n12 months', 'X^2^, 12m', 'df, 12m', 'P, 12m')
...
#### `r table_nums('results_trailb')`
```{r print_results_trailb, results = 'asis'}
pandoc.table(results.trailb,
             justify = c('left', rep('right', ncol(results.trailb) - 1)),
             split.cells = c(15, rep(Inf, ncol(results.trailb) - 1)),
             split.table = Inf,
             keep.line.breaks = TRUE)
...
```{r plots_trailb}
## -- Create data set of predicted vals/CIs for each subtype at various levels of the other --------
predict.hypo.trailb <- map(models.trailb, get.hypo.predict.ols) %>% format.for.cog.plot()
predict.hyper.trailb <- map(models.trailb, get.hyper.predict.ols) %>% format.for.cog.plot()

hypo.trailb.plot <- ggplot(data = predict.hypo.trailb, aes(x = xvar, y = yhat)) +
  facet_grid(time.f ~ facetvar.f) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = nate.navy, alpha = 0.3) +
  geom_line(colour = nate.navy) +
  scale_x_continuous(name = 'Days of hypoactive delirium') +
  scale_y_continuous(name = 'Adjusted Trailmaking B T-Score')

hyper.trailb.plot <- ggplot(data = predict.hyper.trailb, aes(x = xvar, y = yhat)) +
  facet_grid(time.f ~ facetvar.f) +
  geom_pointrange(aes(ymin = lower, ymax = upper), size = 0.3, colour = nate.navy) +
  scale_x_continuous(name = 'Days of hyperactive delirium') +
  scale_y_continuous(name = 'Adjusted Trailmaking B T-Score')
...
#### `r figure_nums('hypo_trailb')`
```{r hypo_trailb_plot, results = 'asis'}
hypo.trailb.plot

```
#### `r figure_nums('hyper_trailb')`
```{r hyper_trailb_plot, results = 'asis'}
hyper.trailb.plot
