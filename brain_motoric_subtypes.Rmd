...
estimate.with.interaction.rms <-
  ...
  summary.text <- sprintf("summary(%s, %s, %s = %s, est.all = FALSE, antilog = FALSE)",
                          model.obj, est.var, int.var, int.adjust.summary)
  ...
  summary.data <- summary.obj %>%
    tidy() %>%
    separate(.rownames, into = c('var.main', 'comp.level', 'ref.level'), sep = " - |:") %>%
...
ital.rows <- function(modresults){
  ital.these <- which(modresults[,'pval'] == '')
  for(i in ital.these){
...
After adjusting for other potential confounders, neither hypoactive or hyperactive delirium on a
given patient-day was associated with a difference in the hazard of death in the twelve months after
enrollment. Results for all model covariates are presented in `r table_nums('results_death_12_all', display = 'cite')`.
