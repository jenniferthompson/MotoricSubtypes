...
hrs.death.inhosp <- pmap(tv.hrs.arglist,
  ...
  bind_rows() %>%
  mutate(var.main.label = label(data.death.inhosp[,var.main]),
...
subtype.plot.death.inhosp <- ggplot(data = hrs.death.inhosp,
  ...
  geom_pointrange(aes(ymin = lowercl, ymax = uppercl)) +
...
hrs.death.12.all <- pmap(tv.hrs.arglist,
  ...
  bind_rows() %>%
  mutate(var.main.label = label(data.death.12.all[,var.main]),
...
## == NEED TO REWORK ESTIMATE.WITH.INTERACTION.RMS() TO WORK WITH CONTINUOUS COVARIATES ============
```
